...
public class CapitalistUI : Form
{
	private IContainer components;

	private DataGridView mContentView;

	private BindingSource mCapitalistBindingSource;
	...
	private void InitializeComponent()
	{
		...
		this.mCapitalistBindingSource = new System.Windows.Forms.BindingSource(this.components);
		...
		((System.ComponentModel.ISupportInitialize)this.mCapitalistBindingSource).BeginInit();
		...
		this.mContentView.DataSource = this.mCapitalistBindingSource;
		...
		this.mCapitalistBindingSource.DataSource = typeof(Oci.Athena.DataSource.Hexin.CapitalistRecord);
		...
		base.Name = "CapitalistUI";
		this.Text = "十大股东";
		((System.ComponentModel.ISupportInitialize)this.mContentView).EndInit();
		((System.ComponentModel.ISupportInitialize)this.mCapitalistBindingSource).EndInit();
	...
	public void Initialize(string filename)
	{
		...
		{
			m_File = default(CapitalistFile);
			flag = CapitalistFile.Read(ref m_File, stream);
		}
		if (!flag)
		{
			return;
	...
	{
		mCapitalistBindingSource.SuspendBinding();
		try
		{
			mCapitalistBindingSource.Clear();
			...
			for (int i = 0; i < complexIndexRecord.RecordNumber; i++)
			{
				...
				{
					mCapitalistBindingSource.Add(m_File.RecordList[i + complexIndexRecord.Position]);
				}
			}
			mCapitalistBindingSource.ResetBindings(metadataChanged: false);
		}
		finally
		{
			mCapitalistBindingSource.ResumeBinding();
		}
	}
}

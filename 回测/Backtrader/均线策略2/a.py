...
class TenDayMASLStrategy(backtrader.Strategy):
    ...
    def __init__(self):
        # 初始化10日简单移动平均线
        self.ma10 = backtrader.indicators.SimpleMovingAverage(
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.data.close[0] > self.ma10[0]:
                ...
                # self.log(f'BUY CREATE, Price: {price:.2f}, MA10: {self.ma10[0]:.2f},可用资金:{cash},买入:{size}')
                # 记录买入订单并执行市价买入
                self.order = self.order_target_percent(target=0.9) # 每次都0.9个
                
        else:
            # **卖出条件1: 收盘价跌破10日均线 (趋势结束)**
            sell_cond1 = self.data.close[0] < self.ma10[0]
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                self.log(f'BUY EXECUTED, Price: {order.executed.price:.2f}, Cost: {order.executed.value:6.2f}, 佣金Comm: {order.executed.comm:.2f},订单完成后 -> 现金: {current_cash:.2f}, 总资产: {portfolio_value:.2f}')
            else:
                # 卖出订单完成，重置买入成本价
                self.buyprice = None
                self.log(f'SELL EXECUTED, Price: {order.executed.price:.2f}, Cost: {order.executed.value:6.2f}, 佣金Comm: {order.executed.comm:.2f},订单完成后 -> 现金: {current_cash:.2f}, 总资产: {portfolio_value:.2f}')
...
if __name__ == '__main__':
    ...
    for i in range(len(codes)):
        try:
            ...
            # 添加策略
            cerebro.addstrategy(TenDayMASLStrategy, ma_period=10, stop_loss=0.10)
            # 加载数据 (请替换为您的数据路径和格式)
            ...
            # 运行回测
            cerebro.run()
            logging.info(f'{code}最终资金: {cerebro.broker.getvalue():.2f}')
    ...
    df.to_csv(os.path.join(file_dir,'result.csv'))
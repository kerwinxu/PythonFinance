...
class TenDayMASLStrategy(backtrader.Strategy):
    ...
    def next(self):
        ...
        if not self.position and len(self.data.close) >= self.params.days:
            ...
            try:
                conds = []
                for i in range(self.params.days):
                    _close = self.data.close[-i] 
                    _sma = self.sma2[-i]
                    if _close is not None and _sma is not None and  _close> _sma:
                        conds.append(1)
                if len(conds) == self.params.days:
...
if __name__ == '__main__':
    ...
    for i in range(len(codes)):
        try:
            ...
            # 添加策略
            cerebro.addstrategy(TenDayMASLStrategy, short=SHORT, long=LONG,stop_loss=STOP_LOSS, days=DAYS)
            # 加载数据 (请替换为您的数据路径和格式)
    ...
    values = df['value']
    logger.info(f'短：{SHORT},长：{LONG}, 止损:{STOP_LOSS},天数:{DAYS},收益平均值:{values.mean()},方差:{values.std()},中位值:{values.median()},最大值：{values.max()},最小值：{values.min()}')
